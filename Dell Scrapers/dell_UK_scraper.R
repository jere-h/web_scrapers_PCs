...
# Dell Product Scraper Type 2 (for UK site)
...
pagelist <- read.delim("dell_UK_urls.txt", stringsAsFactor=F, header=F)
...
compiled <- data.frame(Month="", Brand = 'Dell', LOB="","Dell LOB"="",
                       ...
                       "GBP Price" = pricelist, "Storage Size" = storagesize2, 
                       "Storage Type" = storagetype, "Other Storage Details"="",
                       "Product Type"="", Country="UK")

write.csv(compiled, "dell_UK_products.csv", row.names=FALSE)
