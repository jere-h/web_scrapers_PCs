...
# Dell Product Scraper Type 1 (Malaysia Site)

# Set source folder as working directory
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))

pagelist0 <- read.delim("dell_msia_urls.txt", stringsAsFactor=F, header=F)
...
for (i in 1:nrow(pagelist0)) {
  ...
  for (i in 1:length(pagelist)) {
    ...
    prodname <- currentpage %>% 
      html_node("div.cf-product-order > h1.cf-pg-title > span") %>% 
      html_text(trim=T) %>% 
      na.omit()
...
storagesize2 <- c()
# If Malaysian Page
pricelist <- sub("RM","",pricelist) %>% trimws()
...
compiled <- data.frame(Month="", Brand = 'Dell', LOB="","Dell LOB"="",
                       ...
                       "RM Price" = pricelist, "Storage Size" = storagesize2, 
                       "Storage Type" = storagetype, "Other Storage Details"="",
                       "Product Type"="", Country="Malaysia", URL=urllist)

# Export compiled dataframe as CSV
write.csv(compiled, "dell_msia_products.csv", row.names=FALSE)
