...
# HP Catalogue Scraper (for China site)
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))

pagelist0 <- read.delim("hp_china_urls.txt", stringsAsFactor=F, header=F)
...
for (i in 1:nrow(pagelist0)) {
  ...
  for (i in 1:length(pagelist)) {
    ...
    currprocessor <- currentpage %>% 
      html_node(xpath="//tr[@class='item']/td[@data-th='处理器']") %>% 
      html_text(trim=T)
    
    # Storage details
    currstorage <- currentpage %>% 
      html_node(xpath = "//tr[@class='item']/td[@data-th='硬盘说明']") %>%
      html_text(trim=T)
    
    # Price details
    currprice <- currentpage %>% 
      html_node(xpath="//span[@data-price-type='finalPrice']") %>% 
      html_attr('data-price-amount')
...
storagesize <- sub("(?<=[G|T]).*", "", storagelist, perl = TRUE)
storagetype <- sub(".*?[G|T]", "", storagelist)
storagesize2 <- c()
...
compiled <- data.frame(Month="", Brand = 'HP', LOB="","Dell LOB"="",
                       ...
                       "Yuan Price" = pricelist, "Storage Size" = storagesize, 
                       "Storage Type" = storagetype, "Other Storage Details"="",
                       "Product Type"="", Country="China", URL=urllist)

# Export compiled dataframe as xlsx
write.xlsx(compiled, "hp_china_products.xlsx", row.names=FALSE)
